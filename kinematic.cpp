...
int kinematic::Ik_Find_All_Solutions(std::vector<KDL::JntArray> &found_solutions) {
...
bool kinematic::Ik_Optimal_Solution(const KDL::Frame &target_cart, const KDL::JntArray &current_joints, KDL::JntArray &optimal_joints) {
    ...
    // Temporarily set the target cartesian frame to our class variable so Ik_Find_All_Solutions can use it
    cart = target_cart;

    int num_solutions = Ik_Find_All_Solutions(all_solutions);
